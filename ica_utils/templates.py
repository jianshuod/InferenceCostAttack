        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ":\n"
            return ret
        elif self.sep_style == SeparatorStyle.MOSAIC:
            seps = [self.sep, self.sep2]
            ret = self.system
            for i, (role, message) in enumerate(self.messages):
                if message:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role
            return ret
        elif self.sep_style == SeparatorStyle.WIZARD:
            ret = self.system
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ret += role + message
...
# guanaco-qlora template reported in \url{https://huggingface.co/TheBloke/guanaco-13B-GGML/discussions/1#646f9d3b6098ee820fbd4dba}
...
conv_wizard = Conversation(
    ...
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.WIZARD,
    sep="\n\n",
)
...
# OpenAssistant Pythia default template
conv_stablevicuna = Conversation(
    system="",
    roles=("### Human: ", "### Assistant:"),
    messages=(),
    offset=0,
...
conv_mosaic = Conversation(
    system="Below is an instruction that describes a task. Write a response that appropriately completes the request.\n",
    roles=("### Instruction:", "### Response:"),
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.MOSAIC,
    sep="\n",
    sep2="### End",
)
...
conv_templates = {
    ...
    "stablevicuna":conv_stablevicuna
}
